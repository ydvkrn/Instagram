...
export async function onRequestGet(context) {
    ...
    try {
        const response = await fetch(`${profileJsonUrl}?t=${Date.now()}`, {
        ...
        return new Response(JSON.stringify({ error: error.message, profiles: [] }), {
            status: 500,
            headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
    }
...
export async function onRequestPost(context) {
    ...
    try {
        ...
        // Get SHA
        const shaResponse = await fetch(profileApiUrl, {
            headers: { 
        ...
        // Update
        const updateResponse = await fetch(profileApiUrl, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${GITHUB_TOKEN}`,
                'Accept': 'application/vnd.github.v3+json',
            ...
            body: JSON.stringify({
                message: message || "Update profiles",
